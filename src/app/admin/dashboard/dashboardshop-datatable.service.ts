...
import { SortColumnshop } from './dashboardshop-sortable.directive';



interface SearchResult {
    tables: DashboardShop[];
    total: number;
}
 ...
 */
function sort(tables: DashboardShop[], column: SortColumnshop, direction: string): DashboardShop[] {
    if (direction === '' || column === '') {
        return tables;
    } else {
 ...
 */
function matches(table: DashboardShop, term: string, pipe: PipeTransform) {
    return table.boothName.includes(term)
...
export class DashboardAdvancedService {
    ...
    // tslint:disable-next-line: variable-name
    private _tables$ = new BehaviorSubject<DashboardShop[]>([]);
    // tslint:disable-next-line: variable-name
