...
import { SortDirection, SortColumn } from './meter-sortable.directive';
 ...
 */
function sort(tables: Meter[], column: SortColumn, direction: string): Meter[] {
    if (direction === '' || column === '') {
        return tables;
    } else {
...
function matches(table: Meter, term: string, pipe: PipeTransform) {
    return table.deviceId.includes(term)
...
export class MeterServicemeter {
    ...
    // tslint:disable-next-line: variable-name
    private _tables$ = new BehaviorSubject<Meter[]>([]);
    // tslint:disable-next-line: variable-name
